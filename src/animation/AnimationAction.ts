...
import { Interpolant, PropertyMixer, PropertyBinding } from '../Three';
...
export class AnimationAction {
	...
	_clip : AnimationClip;
	_localRoot : AnimationClip;
	_interpolantSettings : AnimationAction.InterpolantSettings;
	_interpolants : Array<Interpolant>;
	_propertyBindings : Array<PropertyMixer>;

	_cacheIndex : number = null;			// for the memory manager
	_byClipCacheIndex : number = null;		// for the memory manager
	...
	timeScale : number = 1;
	_effectiveTimeScale : number = 1;

	weight : number = 1;
	_effectiveWeight : number = 1;

	repetitions : number = Infinity; 		// no. of repetitions when looping

	paused : boolean = false;				// true -> zero effective time scale
	enabled : boolean = true;				// false -> zero effective weight

	clampWhenFinished : boolean 	= false;	// keep feeding the last frame?
	...
	constructor( mixer : AnimationMixer, clip : AnimationClip, localRoot : AnimationClip = null ){
		...
		let tracks = clip.tracks,
			...
			interpolants = new Array<Interpolant>( nTracks );
...
export module AnimationAction{
	export class InterpolantSettings{
		endingStart : number;
		endingEnd : number;
	}
}